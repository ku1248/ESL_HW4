...
int main(int argc, char *argv[]) {
  ...
  for(int i = 0; i < width; i++){
    for(int j = 0; j < height; j++){
      ...
      *(target_bitmap + bytes_per_pixel * (width * j + i) + 2) = data.uc[0];
      *(target_bitmap + bytes_per_pixel * (width * j + i) + 1) = data.uc[1];
      *(target_bitmap + bytes_per_pixel * (width * j + i) + 0) = data.uc[2];
      //total = (data).sint;
      // if (total - THRESHOLD >= 0) {
      //   // black
      //   *(target_bitmap + bytes_per_pixel * (width * j + i) + 2) = BLACK;
      //   *(target_bitmap + bytes_per_pixel * (width * j + i) + 1) = BLACK;
      //   *(target_bitmap + bytes_per_pixel * (width * j + i) + 0) = BLACK;
      // } else {
      //   // white
      //   *(target_bitmap + bytes_per_pixel * (width * j + i) + 2) = WHITE;
      //   *(target_bitmap + bytes_per_pixel * (width * j + i) + 1) = WHITE;
      //   *(target_bitmap + bytes_per_pixel * (width * j + i) + 0) = WHITE;
      // }
    }
  }
